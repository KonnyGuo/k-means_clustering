...
public class RGBImage {
    ...
    // constructor that reads a jpg OR ppm from disk and loads it into the 2d array
    // pic instance variable
    ...
    public int[] cumulativeHistogram() {
        int h[] = this.histogram();
        int[] cumulativeHist = new int[256];
        int sum = 0;
        int numberOfPixels = this.getNumRows() * this.getNumCols();

        for (int i = 0; i < cumulativeHist.length; i++) {

            sum += h[i] / numberOfPixels;
            cumulativeHist[i] = sum;
        }
        return cumulativeHist;
    }

    public double[] cumulativeHistogramNormRed() {
        int h[] = this.histogramRed();
        ...
        for (int i = 0; i < cumulativeHist.length; i++) {
            ...
            sum += (double) h[i] / numberOfPixels;
            cumulativeHist[i] = sum;
        }
        return cumulativeHist;
    }

    public double[] cumulativeHistogramNormGreen() {
        int h[] = this.histogramGreen();
        double[] cumulativeHist = new double[256];
        double sum = 0;
        int numberOfPixels = this.getNumRows() * this.getNumCols();
    ...
    public double[] cumulativeHistogramNormBlue() {
        int h[] = this.histogramBlue();
        double[] cumulativeHist = new double[256];
        double sum = 0;
        int numberOfPixels = this.getNumRows() * this.getNumCols();
